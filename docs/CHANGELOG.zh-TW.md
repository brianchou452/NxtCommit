...
- 使用作業系統持有的 SQLite 鎖與啟用收據，復原被終止的 worker 並回滾中斷的啟用。
- 拒絕截斷的模型回覆、無效故障輸入及不完整／重複樣本；限制監控回覆大小並隔離合成追蹤紀錄。
- 新增涵蓋三組故障種子及全部八個建議角色的可重現本機 benchmark，不會啟用網站更新。
...
- 新增原子靜態版本切換、過期 epoch 撤銷、切換失敗自動回滾及手動回滾指令。
- 自動修改僅限三個前端請求／狀態檔案；後端、機密、依賴、測試與部署規則仍受保護。
