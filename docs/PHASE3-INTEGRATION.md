...
The statement above records original integration commit `08bead6`. The user
subsequently requested push and merge into main; the following update records
that work without rewriting the original test evidence.

## Main merge follow-up — source 0.7.17

- Pushed `codex/dev-spec-integration` at `08bead6`, then merged it with delivery
  main `7d9fbba`. Preserved the Cloudflare gateway, scoped egress, Responses
  probe, monitoring, two-hour idle and mandatory cutoff.
- Resolved shared environment, README, development, version and changelog
  conflicts. Preserved the optimized CI build and all product test projects;
  included root fixtures in its Docker image and retained interactive reports.
- Production needed both fixture directories and Git. Added them and updated
  the deployment receipt/smoke contract to `phase3-integrated` with the explicit
  `bundled-fixtures-only` boundary. Product advice retains labelled fallbacks in
  the container; the existing Responses credential is not reused for Chat
  Completions and existing cloud egress still excludes GitHub metadata import.
- Verification: Docker CI typechecks/build/version, **99/99 server tests** and
  **38/38 interactive journeys** (56.8 s); 125-spec lint; 4 linter tests; 8 gateway
  tests. Production image smoke uses the actual entrypoint without external
  networking: duration 5/5 → approved, retry 3/3 → needs_review, reset removes
  the authored mission. See `scripts/verify-production-runtime.mjs`.
- The 40 existing visual mismatches remain pending. Live CI, deployment and
  serving-SHA evidence are separate from these local merge checks.
