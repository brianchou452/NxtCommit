...
3. Create a separate candidate. Run typechecks, all server tests, the six frozen
   frontend request contracts, production build, and 12 foundation/Computer C
...
4. The image is pinned by digest. It has no network, service credentials or Docker
   socket; source/tests are read-only, with only output/temp mounts writable.
...
records its immutable ID. Default Docker path is `~/.docker/bin/docker`, context
...
retention management. A crashed process may leave `iteration.lock` or
`control.lock`; verify it is no longer running before removing a stale lock.
...
or authentication claim. Existing missing B runner and C visual approval remain
outside this lane. Live local evidence is recorded separately from controlled
