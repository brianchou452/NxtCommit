...
export const catalogVersion = 'nxtcommit-chaos-v2';
export const fallback = {
  en: 'Inspect the observed evidence; unresolved questions remain open.',
  'zh-TW': '請檢視觀察到的證據；未解問題仍需確認。',
};
...
export interface Measurement {
  ...
  error?: 'scenario_exception';
}

async function modelScenario(id: ScenarioId, signal?: AbortSignal): Promise<Record<string, boolean>> {
  let cancelled = false;
    ...
    const answer =
      id === 'wrong-schema'
        ...
        : id === 'blank-advice'
          ? { en: '  ', 'zh-TW': '\n' }
          : id === 'secret-output'
            ? { en: canary, 'zh-TW': canary }
            : id === 'unsupported-claim'
              ? { en: 'All tests passed and merged.', 'zh-TW': '測試已通過，已合併。' }
              : fallback;
    return Response.json({
      choices: [
        {
          finish_reason: id === 'truncated-completion' ? 'length' : 'stop',
          message: { content: JSON.stringify(answer) },
        },
      ],
      usage: { prompt_tokens: id === 'invalid-usage' ? -1 : 5, completion_tokens: 6, total_tokens: 11 },
    });
  };
  const sink: ObservationSink = {
    async trace(record) {
      traceSeen = true;
      traceClean =
        !JSON.stringify(record).includes(canary) && !JSON.stringify(record).includes('PRIVATE_EVIDENCE');
  ...
  const assistance = new Assistance(configuration, sink, transport, signal);
  ...
  const expectModel = ['healthy-model', 'trace-unavailable', 'trace-containment', 'invalid-usage'].includes(
    id,
  );
  return {
    unknownUsage: id !== 'invalid-usage' || result.evidence.usage === undefined,
...
async function applicationScenario(id: ScenarioId, signal?: AbortSignal): Promise<Record<string, boolean>> {
  let workerReady = false;
  const application = createApplication({
    configuredMode: 'demo',
    operations: { runDispatchMode: 'queue', workerReady: () => workerReady },
  });
  ...
  try {
    if (id === 'malformed-request' || id === 'oversized-request') {
      const response = await fetch(`${url}/api/analyze`, {
        method: 'POST',
        headers: { 'content-type': 'application/json' },
        body: id === 'malformed-request' ? '{' : JSON.stringify({ data: 'x'.repeat(40_000) }),
        signal: AbortSignal.any([AbortSignal.timeout(3000), ...(signal ? [signal] : [])]),
      });
      const body = (await response.json()) as { code: string };
      return {
        rejected: response.status === (id === 'malformed-request' ? 400 : 413),
        stableError: body.code === (id === 'malformed-request' ? 'invalid_json' : 'request_too_large'),
      };
    }
    if (id === 'reset-revokes-capability') {
      const { analysis } = (await (await post('/api/analyze', { source: 'fixture' })).json()) as {
        analysis: { serverToken: string; issues: Array<{ id: string }> };
      };
      const response = await post('/api/demo/reset', {});
      await response.arrayBuffer();
      const stale = await post('/api/analysis/assist', { analysis, issueId: analysis.issues[0]!.id });
      await stale.arrayBuffer();
      const fresh = await post('/api/analyze', { source: 'fixture' });
    ...
    if (id === 'database-unavailable') {
      workerReady = true;
      application.context.store.db.exec(
        'DROP TABLE authored_demo_evidence; DROP TABLE authored_review_decisions; DROP TABLE authored_missions',
      );
    }
    ...
    if (id === 'worker-recovery') {
      workerReady = true;
      const recovered = await fetch(`${url}/readyz`, {
        signal: AbortSignal.any([AbortSignal.timeout(3000), ...(signal ? [signal] : [])]),
      });
    ...
    return {
      degraded: degraded.status === 503,
      databaseFailureObserved: readiness.db === false,
      workerStillReady: readiness.workerReady === true,
      livenessIndependent: health.ok,
    };
  } finally {
...
export class ChaosAgent {
  async execute(scenario: ScenarioId, repetition: number, signal?: AbortSignal): Promise<Measurement> {
    if (
    ...
    try {
      const checks =
        scenarioIds.indexOf(scenario) < scenarioIds.indexOf('malformed-request')
          ? await modelScenario(scenario, signal)
          : await applicationScenario(scenario, signal);
      return {
        scenario,
        repetition,
      ...
      if (signal?.aborted) throw new DOMException('Agent interrupted', 'AbortError');
      return {
        scenario,
        repetition,
        passed: false,
