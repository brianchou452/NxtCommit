...
function setup() {
  const root = mkdtempSync(join(tmpdir(), 'nxtcommit-update-test-'));
...
test('self-update starts disabled and demo-locked; enabling alone cannot update', async () => {
  const s = setup();
  try {
    assert.equal(s.control.read().enabled, false);
    assert.equal(s.control.read().demoLocked, true);
    const settings = await s.control.change({ enabled: true });
    assert.equal(s.control.allowed(settings.epoch), false);
...
test('demo-on and off/on both revoke an in-flight update epoch', async () => {
  ...
  try {
    ...
    await s.control.change({ enabled: false, demoLocked: false });
...
test('failed post-switch smoke rolls back to exactly the previous release', async () => {
  ...
  try {
    ...
    assert.equal(
      await s.control.promote(settings.epoch, s.base, s.candidate, async () => false),
      'rolled-back',
    );
    assert.equal(s.control.active(), s.base);
    assert.equal(
      await s.control.promote(settings.epoch, s.base, s.candidate, async () => {
...
test('successful promotion replay returns its receipt without switching again', async () => {
  const s = setup();
  try {
    const settings = await s.control.change({ enabled: true, demoLocked: false });
    ...
    assert.equal(
      await s.control.promote(settings.epoch, s.base, s.candidate, async () => {
        throw Error('must not repeat smoke');
      }),
      'promoted',
    );
  } finally {
...
test('acknowledged demo lock serializes against activation and blocks subsequent switches', async () => {
  ...
  try {
    const settings = await s.control.change({ enabled: true, demoLocked: false });
    let release!: () => void;
    const pending = s.control.promote(
      settings.epoch,
      s.base,
      s.candidate,
      () =>
    ...
    const freeze = s.control.change({ demoLocked: true });
    release();
    await pending;
    await freeze;
    assert.equal(s.control.read().demoLocked, true);
...
const sources = Object.fromEntries(
  editableFiles.map((path) => [path, 'export const value = 1;']),
) as SourceFiles;
const proposal = (path: string, before = '1', after = '2') => ({
...
test('proposal accepts a bounded exact edit but rejects protected files and traversal', () => {
  ...
  for (const path of [
    '../../.env',
    '/tmp/code.ts',
    'server/self-update/control.ts',
    'server/authoring/assistance.ts',
    'package.json',
    'server/self-update/control.test.ts',
  ])
    assert.throws(() => validateProposal(proposal(path), sources));
});
test('proposal rejects ambiguous edits, remote URLs, secrets, blank replacements and new imports', () => {
  ...
  for (const after of [
    ...
    'sk-SYNTHETIC_NOT_A_REAL_SECRET_000000',
    'eval("x")',
    '1;\nimport x from "somewhere";',
  ])
    assert.throws(() => validateProposal(proposal(editableFiles[0], '1', after), sources));
  const duplicate = proposal(editableFiles[0]);
  duplicate.edits.push(duplicate.edits[0]!);
  assert.throws(() => validateProposal(duplicate, sources));
});
test('deployment smoke rejects a healthy app that is not serving the selected release', async () => {
  ...
  const s = setup();
  let header = 'wrong-release';
  const server = createServer((request, response) => {
    response.setHeader('X-NxtCommit-Static-Release', header);
    response.end(request.url === '/readyz' ? '{"status":"ok"}' : s.base);
  });
  server.listen(0, '127.0.0.1');
  await once(server, 'listening');
  const port = (server.address() as { port: number }).port;
  writeFileSync(
    join(s.root, 'runtime.json'),
    JSON.stringify({
  ...
  try {
    assert.equal(await smoke(s.control), false);
    header = s.base;
    assert.equal(await smoke(s.control), true);
  } finally {
