...
export function authoringConfiguration(env: NodeJS.ProcessEnv): AuthoringOptions {
  ...
  if (env.OPENAI_API_KEY) {
    ...
    result.model = { apiKey: env.OPENAI_API_KEY, model: env.OPENAI_MODEL || 'gpt-5-mini', baseUrl, api: baseUrl.replace(/\/$/, '') === 'https://api.openai.com/v1' ? 'responses' : 'chat-completions' };
  }
