import { CatalogContent } from '../components/CatalogContent.js';
...
function MissionDetailContent({ id }: { id: string }) {
  ...
  const canPledge = (mission.status === 'funding' || (mission.status === 'stalled' && !!mission.catalog)) && (mission.project.workspace.kind !== 'none' || !!mission.catalog) && mission.computePledged < mission.computeGoal;
  ...
  return <div className="mission-page">
    ...
    {mission.catalog && <p className="mission-provenance">{text.catalog_note}</p>}
    ...
    <div className="mission-columns"><article className="mission-narrative">
      ...
      <CatalogContent mission={mission} />
      <ProjectExplanationRegion key={mission.project.id} path={`/api/projects/${encodeURIComponent(mission.project.id)}/explain`} />
      <section className="mission-story-section" id="mission-delivery"><p className="mission-kicker">{text.mission_evidence}</p><h2>{text.mission_delivery}</h2><div className="mission-delivery-metrics"><div><span>{text.mission_pledged}</span><strong>{mission.computePledged.toLocaleString()}</strong></div><div><span>{text.mission_used}</span><strong>{mission.computeConsumed.toLocaleString()}</strong></div><div><span>{text.mission_backers}</span><strong>{(mission.backerCount ?? new Set(mission.pledges.map(pledge => pledge.contributorId)).size)}</strong></div></div><div className="mission-delivery-track">{(['funding', 'development', 'verification', 'adoption'] as const).map(dimension => <div className={`mission-track-${dimension}`} key={dimension}><span>{text[`mission_${dimension}`]}</span><strong>{(mission.progress[dimension] * 100).toFixed(0)}%</strong><progress aria-label={text[`mission_${dimension}`]} max={1} value={mission.progress[dimension]} /></div>)}</div><p className="mission-provenance">{text.mission_units} · {text.mission_data}: {mission.dataMode}</p>{mission.latestRun ? <Link className="mission-evidence-link" to={runLink}>{text.mission_history} →</Link> : <p>{text.mission_idle_note}</p>}{mission.artifact?.testEvidenceSource === 'demo' && <p className="mission-provenance">{text.mission_seed}</p>}</section>
      ...
      {mission.status === 'executing' && mission.latestRunId && <Link className="mission-button" to={runLink}>{text.mission_watch}</Link>}
      {mission.status === 'needs_review' && mission.latestRunId && <Link className="mission-button" data-guide-target="next" data-guide-step="4" data-guide-title="mission_review" to={`/missions/${encodeURIComponent(id)}/review${guide}`}>{text.mission_review}</Link>}
      {['approved', 'changes_requested', 'failed', 'stalled'].includes(mission.status) && mission.latestRunId && <Link to={runLink}>{text.mission_history}</Link>}
      {mission.status === 'released' && mission.latestRunId && <Link to={runLink}>{text.mission_release}</Link>}
      {mission.catalog && <a href="#catalog-history">{text.catalog_scope}</a>}
